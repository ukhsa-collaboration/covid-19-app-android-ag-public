...
import uk.nhs.nhsx.covid19.android.app.databinding.ActivityOrderLfdTestBinding
...
class OrderLfdTestActivity : BaseActivity(R.layout.activity_order_lfd_test) {
    ...
    private lateinit var binding: ActivityOrderLfdTestBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        appComponent.inject(this)
        binding = ActivityOrderLfdTestBinding.inflate(layoutInflater)
        with(binding) {
            setContentView(root)

            setNavigateUpToolbar(
                primaryToolbar.toolbar,
                R.string.book_free_lfd_test_title,
                upIndicator = R.drawable.ic_arrow_back_white
            )

            orderTestButton.setOnSingleClickListener { viewModel.onOrderTestClicked() }

            orderTestButton.setUpLinkTypeWithBrowserWarning(orderTestButton.text)

            alreadyHaveTestKitButton.setOnSingleClickListener { viewModel.onAlreadyHaveTestKitClicked() }
        }
