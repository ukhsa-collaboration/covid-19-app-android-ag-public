...
class HandleInitialExposureNotification @Inject constructor(
    ...
    suspend operator fun invoke(token: String): Result<InitialCircuitBreakerResult> =
        runSafely {
            exposureWindowRiskManager.getRisk(token)?.let { riskyExposureInfo ->
                    ...
                    .submitExposureInfo(
                        ExposureCircuitBreakerRequest(
                            ...
                            daysSinceLastExposure = Instant.ofEpochMilli(riskyExposureInfo.startOfDayMillis)
                                .until(Instant.now(clock), ChronoUnit.DAYS).toInt(),
                            matchedKeyCount = 1,
                            riskCalculationVersion = riskyExposureInfo.riskCalculationVersion
                        )
                    )

                return@runSafely when (CircuitBreakerResult.valueOf(response.approval.toUpperCase(Locale.ROOT))) {
