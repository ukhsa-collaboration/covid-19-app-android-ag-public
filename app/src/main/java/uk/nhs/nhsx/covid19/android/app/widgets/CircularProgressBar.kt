...
import uk.nhs.nhsx.covid19.android.app.widgets.CircularProgressBar.GradientDirection.BOTTOM_TO_END
import uk.nhs.nhsx.covid19.android.app.widgets.CircularProgressBar.GradientDirection.LEFT_TO_RIGHT
import uk.nhs.nhsx.covid19.android.app.widgets.CircularProgressBar.GradientDirection.RIGHT_TO_LEFT
import uk.nhs.nhsx.covid19.android.app.widgets.CircularProgressBar.GradientDirection.TOP_TO_BOTTOM
import uk.nhs.nhsx.covid19.android.app.widgets.CircularProgressBar.ProgressDirection.TO_LEFT
import uk.nhs.nhsx.covid19.android.app.widgets.CircularProgressBar.ProgressDirection.TO_RIGHT
...
class CircularProgressBar(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    var progressBarColorDirection: GradientDirection =
        LEFT_TO_RIGHT
        set(value) {
                field = value
                manageColor()
    ...
    var backgroundProgressBarColorDirection: GradientDirection =
        LEFT_TO_RIGHT
        set(value) {
                field = value
                manageBackgroundProgressBarColor()
    ...
    private var progressDirectionIndeterminateMode: ProgressDirection =
        TO_RIGHT
        set(value) {
                field = value
                invalidate()
            }
    private var startAngleIndeterminateMode: Float =
        DEFAULT_START_ANGLE
        set(value) {
                field = value
                invalidate()
    ...
    private fun createLinearGradient(startColor: Int, endColor: Int, gradientDirection: GradientDirection): LinearGradient {
        ...
        when (gradientDirection) {
            LEFT_TO_RIGHT -> x1 = width.toFloat()
            RIGHT_TO_LEFT -> x0 = width.toFloat()
            TOP_TO_BOTTOM -> y1 = height.toFloat()
            BOTTOM_TO_END -> y0 = height.toFloat()
        }
    ...
    private fun ProgressDirection.reverse(): ProgressDirection =
        if (this.isToRight()) TO_LEFT else TO_RIGHT

    private fun ProgressDirection.isToRight(): Boolean = this == TO_RIGHT
